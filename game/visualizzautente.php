...
if($u['n']>0){
    ...
    $sfida=0;
    if($datiutente['ultimazione']>($adesso-600)){
        $eventsuv=$db->QuerySelect("SELECT COUNT(id) AS id FROM eventi WHERE userid='".$utente."'");
        // if both users not have are not doing anything
        if ($eventi['id']==0 AND $eventsuv['id']==0)
            $sfida=1;
    }
}//se utente esiste
